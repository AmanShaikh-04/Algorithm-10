...
const Team_card = ({ image_src, role, name }) => (
  <div className="group flex h-[168px] w-32 flex-col items-center rounded-sm bg-[#f9f9f9] p-2 drop-shadow-[0_15px_30px_rgba(255,140,66,0.3)] transition-all duration-300 hover:-translate-y-3 hover:scale-105 hover:drop-shadow-[0_20px_40px_rgba(255,140,66,0.7)]">
    <div className="relative h-[110px] w-full overflow-hidden bg-[#222]">
      <Image
        src={image_src}
        alt={name}
        fill
    ...
    <div className="mt-auto flex w-full flex-col items-center justify-center pb-1 text-gray-800">
      <span className="mb-[2px] font-[cursive] text-[20px] leading-none text-[#1a1a1a]">
        {name}
      </span>
      <span className="text-[8px] font-bold tracking-widest text-gray-500 uppercase transition-colors duration-300 group-hover:text-orange-600">
        {role}
      </span>
    </div>
  </div>
...
const linkedin_svg = () => (
  <svg viewBox="0 0 24 24" className="h-full w-full fill-current">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24.774 24 23.2 0 22.225 0z" />
  </svg>
);
const youtube_svg = () => (
  <svg viewBox="0 0 24 24" className="h-full w-full fill-current">
...
const gmail_svg = () => (
  <svg viewBox="0 0 24 24" className="h-full w-full fill-current">
    <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L12 9.59l8.073-6.097C21.691 2.279 24 3.434 24 5.457z" />
  </svg>
);
...
const Footer = () => {
  ...
  const smooth_rotation = useSpring(added_rotation, {
    damping: 50,
    stiffness: 400,
  });
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      ...
      if (atBottom) {
        added_rotation.set(
          added_rotation.get() - (lastY - e.touches[0].clientY) * 0.1
        );
        lastY = e.touches[0].clientY;
      }
    };
  ...
  return (
    <footer className="relative flex h-[100dvh] min-h-[600px] w-full flex-col items-center overflow-hidden border-t border-orange-500/20 bg-[#050505]/80 text-white shadow-[0_-15px_40px_rgba(255,140,66,0.1)]">
      {/* Background */}
      <div className="pointer-events-none absolute inset-0 z-0 opacity-60">
        <SharedBackground id="footer-particles" />
      </div>
      <div className="pointer-events-none absolute bottom-0 left-1/2 z-0 h-[400px] w-[800px] -translate-x-1/2 translate-y-1/2 rounded-full bg-gradient-to-t from-orange-600/20 to-transparent blur-[120px]" />

      {/* ── Arc ── */}
      <div
        className="pointer-events-none absolute left-1/2 z-10 -translate-x-1/2 opacity-90"
        style={{ width: arcW, height: arcH, top: arcTop }}
      >
        <motion.div
          style={{ rotate: smooth_rotation }}
          className="relative h-full w-full"
        >
          <motion.div
            animate={{ rotate: -360 }}
            transition={{
              duration: mobile ? 120 : 1000,
              repeat: Infinity,
              ease: "linear",
            }}
            className="relative h-full w-full"
          >
            {base_team_members.map((member, i) => (
              <div
                ...
                style={{
                  transform: `rotate(${i * (360 / base_team_members.length)}deg)`,
                }}
              >
                <div className="pointer-events-auto absolute top-0 left-1/2 -translate-x-1/2 cursor-pointer transition-transform duration-300 hover:-translate-y-4 hover:scale-110">
      ...
      {/* ── Bottom UI ── */}
      <div className="relative z-20 mt-auto flex w-full max-w-4xl flex-col items-center px-4 pb-6">
        <div className="mb-6 flex w-full flex-col items-center gap-6">
          {/* Map */}
          <div
            className="group relative w-full max-w-[450px]"
            style={{ height: `${cfg.mapH}px` }}
          >
            <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500 opacity-30 blur transition duration-500 group-hover:opacity-60" />
            <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-1 shadow-[0_0_30px_rgba(0,0,0,0.8)] backdrop-blur-xl transition-all duration-500 hover:-translate-y-1">
              <iframe
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {social_items.map((item) => (
              <a
                ...
                className={`flex cursor-pointer items-center justify-center rounded-2xl border transition-all duration-300 hover:-translate-y-2 ${item.colors}`}
                style={{ width: cfg.iconW, height: cfg.iconW }}
                title={item.name}
              >
                <div style={{ width: cfg.iconSz, height: cfg.iconSz }}>
                  <item.icon />
                </div>
              </a>
            ))}
        ...
        <div className="flex w-full flex-col items-center text-center">
          <div className="mt-2 mb-4 flex w-full max-w-2xl items-center justify-center">
            <div className="h-px w-full bg-gradient-to-r from-transparent via-orange-500/50 to-transparent" />
            <div className="mx-4 h-1.5 w-1.5 animate-pulse rounded-full bg-orange-500 shadow-[0_0_10px_rgba(255,140,66,0.8)]" />
            <div className="h-px w-full bg-gradient-to-l from-transparent via-amber-500/50 to-transparent" />
          </div>
          <p className="flex items-center justify-center gap-1.5 text-sm font-normal tracking-wide text-white/90 lg:text-base">
            Made With{" "}
            <span className="inline-block animate-pulse text-lg text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.5)] lg:text-xl">
              ❤️
            </span>{" "}
            By
            <span className="animate-[gradient_3s_linear_infinite] bg-gradient-to-r from-orange-400 via-amber-400 to-orange-400 bg-[length:200%_auto] bg-clip-text font-bold text-transparent">
              Algo Team
            </span>
          </p>
